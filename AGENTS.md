...
**`hideVaultName` is the fix, and it is keyed on the vault name rather than on a class of Obsidian's chrome.** Before each capture the desktop suite calls Obsidian's `hide()` on every leaf element whose `textContent` is `app.vault.getName()` — today that is the vault-switcher row at the bottom of the left sidedock — and `shoot` asserts the count is non-zero, so an Obsidian that relocates that row fails the suite loudly instead of quietly going back to rewriting the PNGs. The varying thing is the DATA, and that row has moved between versions before, which is why no selector is hardcoded. Four runs with four different vault names then produced identical bytes. `hide()` rather than an inline `style.visibility` or an injected stylesheet, because `obsidianmd/no-static-styles-assignment` and `obsidianmd/no-forbidden-elements` refuse those respectively. **This is a local copy of a fix that belongs in `obsidian-integration-testing`**: any plugin capturing a desktop frame through that harness has the same churn, and the local copy is meant to be deleted once the harness makes the capture deterministic itself.
