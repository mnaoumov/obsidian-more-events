...
/*
 * These four values are a WIRE CONTRACT: a consumer hardcodes the event names rather than importing them,
 * and negotiates against the version. So the test asserts the literals themselves — a rename that compiles
...
describe('the published contract', () => {
  ...
  it('should declare every member of the api in the contract', () => {
    expect(Object.keys(PLUGIN_API_CONTRACT).sort()).toEqual(['getEnabledCorePluginIds', 'isCorePluginEnabled']);
  });
